...
export const FavoritesProvider: React.FC<FavoritesProviderProps> = ({ children }) => {
    ...
    const fetchArtistInfo = async (id: string): Promise<Artist | null> => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/artist/${id}`, {
                method: 'GET',
                headers: {
    ...
    const toggleFavorite = async (artist: Artist | string, image?: string): Promise<void> => {
        ...
        if (typeof artist === 'string') {
            ...
            artistImage = image || ''; // Use provided image
        } else {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/addToFavorites`, {
                method: "POST",
