...
const Favorites: React.FC = () => {
  ...
  const remove = async (id: string, artistName: string) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/removeFavorites?id=${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (response.ok) {
  ...
  const handleArtistClick = async (artist: Artist, event: React.MouseEvent) => {
    ...
    try {
      ...
      if (cachedData) {
        ...
        const response = await fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/artist/${artist.artistId}`, {
          method: 'GET',
          headers: {
            'X-XAPP-Token': token
          }
